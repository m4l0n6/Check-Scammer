...
export const validationSchema = z.object({
    scammer__name: z
      ...
      .nonempty({ message: "Nhập tên tài khoản" }),
    scammer_phone: z
      .string()
      .regex(/^\d+$/, { message: "Chỉ nhập số" })
      .min(10, { message: "Nhập tối thiêu 10 ký tự" })
      .nonempty({ message: "Nhập số điện thoại" }),
    bank__number: z
      .string()
      .regex(/^\d+$/, { message: "Chỉ nhập số" })
      .nonempty({ message: "Nhập số tài khoản" }),
    bank__name: z.string().min(3, { message: "Nhập tối thiểu 3 ký tự" }).nonempty({ message: "Nhập tên ngân hàng" }),
